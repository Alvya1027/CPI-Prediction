...
老师模型内部自带 `4 us` 的输入 Transport Delay。Python/MATLAB 生成的输入从 0 秒开始，经过模型内部延迟后在 4 微秒处进入储备池，因此状态从 `4 us` 开始按 40 ps 间隔提取。
...
普通光储备池公平对照使用同一批状态直接预测 CPI：

```bash
python -m src.optical_reservoir_regression
```

该脚本会额外生成普通光储备池结果，以及普通/孪生模型的统一指标表和测试集对比图。
