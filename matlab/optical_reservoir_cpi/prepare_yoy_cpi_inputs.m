function summary = prepare_yoy_cpi_inputs(config)
%PREPARE_YOY_CPI_INPUTS Build fixed-mask inputs for YoY train/test only.
%
% This is the no-validation counterpart of prepare_cpi_inputs.  It reads the
% continuous 12-month windows prepared from cpi_data_lastyear=100.csv and
% writes only train/test Simulink input files.
...
assert(double(data.window_size) == config.window_size, ...
    'The exported YoY window size does not match config_cpi_rc.m.');
