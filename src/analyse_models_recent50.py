...
file_list = [
    ...
    "recent50_test_prediction_comparison.csv",
    "recent50_model_comparison.csv"
]
...
reservoir_pred = pd.read_csv(file_paths["recent50_test_prediction_comparison.csv"])
reservoir_metrics = pd.read_csv(file_paths["recent50_model_comparison.csv"])
...
# 输出目录（专门用于开放参考库对比）
OUTPUT_DIR = os.path.join(base_dir, "results", "figures", "recent50")
...
siamese_test = reservoir_pred[['sample_i_id', 'cpi_actual', 'cpi_predicted_siamese_recent50']].copy()
...
siamese_test = siamese_test.rename(columns={'sample_i_id': 'sample_id', 'cpi_actual': 'actual',
                                            'cpi_predicted_siamese_recent50': 'predicted'})
...
# 储备池指标（注意模型名称）
ordinary_val_rmse = reservoir_metrics[reservoir_metrics['model'] == 'ordinary_optical_reservoir_recent50']['val_rmse'].values[0]
ordinary_test_rmse = reservoir_metrics[reservoir_metrics['model'] == 'ordinary_optical_reservoir_recent50']['test_rmse'].values[0]
siamese_val_rmse = reservoir_metrics[reservoir_metrics['model'] == 'siamese_optical_reservoir_recent50_targets']['val_rmse'].values[0]
siamese_test_rmse = reservoir_metrics[reservoir_metrics['model'] == 'siamese_optical_reservoir_recent50_targets']['test_rmse'].values[0]
...
plt.plot(all_test['sample_id'], all_test['ordinary'], 'm:', label='Ordinary Reservoir (recent50)', marker='d')
plt.plot(all_test['sample_id'], all_test['siamese'], 'r-', label='Siamese Reservoir (recent50)', marker='x')
plt.xlabel('Sample ID (Test Months)')
plt.ylabel('CPI')
plt.title('CPI Prediction Curves on Test Set (Open Reference Library)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'prediction_curves_recent50.png'), dpi=300)
plt.close()
...
plt.title('Residual Distribution of Four Models (Open Reference)')
plt.grid(True, axis='y')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'residual_boxplot_recent50.png'), dpi=300)
plt.close()
...
plt.title('Validation vs Test RMSE (Generalization, Open Reference)')
plt.ylabel('RMSE')
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'generalization_rmse_recent50.png'), dpi=300)
plt.close()
...
plt.title('Top 5 Test Months with Largest Average Absolute Error (Open Reference)')
plt.xlabel('Sample ID')
plt.ylabel('Absolute Error')
plt.legend(title='Model')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'top5_errors_recent50.png'), dpi=300)
plt.close()

print("Top 5 months with largest average absolute errors (Open Reference):")
print(top5_samples)
...
accuracies = {name: directional_accuracy(pred.values, all_test['actual'].values) for name, pred in models_dict.items()}
...
plt.title('Trend Direction Prediction Accuracy (Open Reference)')
for i, row in acc_df.iterrows():
    plt.text(i, row['Accuracy'] + 0.02, f"{row['Accuracy']:.2%}", ha='center', va='bottom')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'directional_accuracy_recent50.png'), dpi=300)
plt.close()

# 饼图（准确率）
plt.figure(figsize=(8, 8))
plt.pie(accuracies.values(), labels=accuracies.keys(), autopct='%1.1f%%', startangle=90)
plt.title('Directional Accuracy Comparison (Open Reference)')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'directional_accuracy_pie_recent50.png'), dpi=300)
plt.close()
